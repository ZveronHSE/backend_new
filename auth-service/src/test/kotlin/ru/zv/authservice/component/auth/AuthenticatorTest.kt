...
import ru.zv.authservice.util.randomDecodedToken
import ru.zv.authservice.util.randomDeviceFp
import ru.zv.authservice.util.randomName
import ru.zv.authservice.util.randomSessionEntity
...
class AuthenticatorTest {
    ...
    fun `when refresh mobile session and session storage throws exception, then throw exception`(): Unit = runBlocking {
        ...
        coEvery { jwtManager.decodeRefreshToken(eq(token)) } returns decodedToken
        coEvery { profileServiceClient.getProfileById(eq(decodedToken.profileId)) } returns profile
        coEvery { sessionStorage.updateSession(eq(decodedToken.sessionId), eq(fp), decodedToken.tokenIdentifier) } throws SessionExpiredException()
